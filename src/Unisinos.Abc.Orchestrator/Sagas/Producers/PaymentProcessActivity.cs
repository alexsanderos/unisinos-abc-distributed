...
{
    public class PaymentProcessActivity : IStateMachineActivity<PurchaseCourseState>
    {
        private readonly ITopicProducer<PaymentProcessCommand> _topicProducer;
        private readonly ILogger<PaymentProcessActivity> _logger;

        public PaymentProcessActivity(
            IServiceScopeFactory serviceScopeFactory,
            ILogger<PaymentProcessActivity> logger)
        {
            var scope = serviceScopeFactory.CreateScope();
            _topicProducer = scope.ServiceProvider.GetService<ITopicProducer<PaymentProcessCommand>>();
            _logger = logger;
        }
        ...
        {
            await Execute((BehaviorContext<PurchaseCourseState, IApprovedProfileEvent>)context);
        ...
        {
            context.CreateScope(typeof(PaymentProcessCommand).ToString());
        }

        async Task Execute(BehaviorContext<PurchaseCourseState, IApprovedProfileEvent> context)
        {
            _logger.LogInformation("Send message payment process to broker correlationId:{0}", context.Message.CorrelationId);
